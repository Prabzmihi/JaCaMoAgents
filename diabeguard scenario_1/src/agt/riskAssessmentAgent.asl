...
+!getLifestyleMedications : true <-
    ...
    .print("Risk Assessment Agent sent risk and genetic scores to Lifestyle Recommandation Agent").

+recommanded(LifestyleChanges, Medication)[source(AGENT)] : true <-
    -+lifestyleRecommandation(LifestyleChanges);
    -+medicationRecommandation(Medication);
    .print("Recieved lifestyle changes and medications from ", AGENT).
...
+!send_recommandation_back : true <-
    ...
    .print("Sent lifestyle and medication recommandations to John Agent").
